...
class ORMFixture:
    ...
    class ORMGroup(db.Entity):
        ...
        name = Optional(str, column='group_id')
        header = Optional(str, column='group_id')
        footer = Optional(str, column='group_id')
    ...
    def get_contact_list(self):
        return self.convert_contacts_to_model(select(c for c in ORMFixture.ORMContact if
                                                     c.deprecated is None))
    @db_session
    ...
    def get_contacts_not_in_group(self,group):
        orm_group=list(select(g for g in ORMFixture.ORMGroup if g.id==group.id))[0]
        return self.convert_contacts_to_model(select(c for c in ORMFixture.ORMContact if
                                                     c.deprecated is None and orm_group not in c.groups))
